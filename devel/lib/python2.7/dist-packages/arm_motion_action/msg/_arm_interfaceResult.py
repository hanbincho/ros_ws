...
"""
  ...
  def serialize(self, buff):
    ...
    try:
      ...
      pattern = '<%sd'%length
      buff.write(struct.pack(pattern, *self.segmented_path_arrival_times))
      _x = self.err_msg
      length = len(_x)
      ...
      buff.write(_get_struct_d().pack(self.computed_arrival_time))
      length = len(self.q_arm)
      buff.write(_struct_I.pack(length))
      pattern = '<%sd'%length
      buff.write(struct.pack(pattern, *self.q_arm))
      _x = self
  ...
  def deserialize(self, str):
    ...
    try:
      ...
      self.segmented_path_arrival_times = struct.unpack(pattern, str[start:end])
      start = end
      end += 4
      (length,) = _struct_I.unpack(str[start:end])
      start = end
      ...
      self.q_arm = struct.unpack(pattern, str[start:end])
      _x = self
      start = end
      end += 12
      ...
      if python3:
        self.current_pose_gripper.header.frame_id = str[start:end].decode('utf-8')
      else:
